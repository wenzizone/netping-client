...
my $year = `date -d '-1days' +%Y`;
my $month = `date -d '-1days' +%m`;
my $day = `date -d '-1days' +%d`;
chomp($year);
chomp($month);
chomp($day);

my $file_name_suffix = '_'.$year.'_'.$month.'_'.$day.".log";

# -----MAIN------

my $cfg = new Config::IniFiles(-file => $base_dir."/config.ini");

my $dest_dir = $cfg->val('file','dest_dir');

my $log_dir_mid = $year."/".$month."/";
...
if ($option eq 'put') {
    ...
    foreach (@idc_groups) {
        ...
        $remote_file = $log_dir."/".$log_dir_mid.$host.$file_name_suffix;
